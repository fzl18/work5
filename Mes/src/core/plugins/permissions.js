...
import { setEventTrack } from '@/utils/eventTrack'
...
router.beforeEach(async (to, from, next) => {
  ...
  const dictionary = store.getters['coreDictionary/dictionary']
  if (showProgressBar) NProgress.start()
  //埋点
  setEventTrack(store.getters['coreCommon/userinfo'], to)
  // 看是否有本地token
  ...
  if (!loginInterception) {
    ...
    if (!dictionary.length) {
      await store.dispatch('coreDictionary/setDictionary')
    }
    if (hasRoutes) next()
  }
  //如果白名单，直接放行

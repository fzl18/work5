<template>
  <div class="app-container">
    ...
    >
      <template slot="drawer">
        <el-form :model="queryForm" label-position="top">
          <el-form-item :label="$translateTitle('SOP名称')">
    ...
    <PopSop :visible.sync="visibleSop" @refresh="querySearch" />
  </div>
</template>
<script>
  import PopSop from '../components/PopSop.vue'
  import { getSopList, delSop, getSopUrl } from '../api'
  export default {
    name: 'Sop',
    components: {
      PopSop,
    ...
    methods: {
      ...
      async download(row) {
        let downloadUrl = await getSopUrl({ id: row.id })
        if (downloadUrl) {

  ...
  <div class="app-container">
    ...
    >
      <template slot="drawer">
        <el-form :model="queryForm" label-position="top">
          <el-form-item prop="unitUnionCode" :label="$translateTitle('公司')">
            <el-select
              v-model="queryForm.unitUnionCode"
              clearable
          ...
          <el-form-item :label="$translateTitle('编码/名称')">
            <el-input v-model="queryForm.materialName" clearable></el-input>
          </el-form-item>
          <el-form-item :label="$translateTitle('规格型号')">
            <el-input v-model="queryForm.mspecific" clearable></el-input>
          </el-form-item>
          <el-form-item
            prop="materialGroup"
            :label="$translateTitle('产品类别')"
          >
            <el-cascader
              v-model="queryForm.materialGroup"
              :options="typeOptions"
              :props="typeProps"
              clearable
              style="width: 100%"
      ...
      <div class="my-header">
        <div class="header">
          ...
          <div class="btns">
            ...
            <!-- <bt-button type="primary" @click="showSetting">
              {{ $translateTitle('展示列配置') }}
            </bt-button> -->
          </div>
        </div>
        <div class="line"></div>
      ...
      >
        ...
        <el-table-column
          prop="suppleUnit"
          :label="$translateTitle('单位')"
          min-width="80"
        />
        <el-table-column
          prop="materialType"
          :label="$translateTitle('产品类型')"
          min-width="150"
        />
        <el-table-column
          fixed="right"
        ...
        >
          <template #default="{ row }">
            <bt-dropdown :show-split="false">
              <bt-dropdown-menu>
                <bt-dropdown-item @click="gotoSOP(row)">
                  <i class="el-icon-document"></i>
                  {{ $translateTitle('SOP') }}
                </bt-dropdown-item>
                <bt-dropdown-item @click="gotoDocument(row)">
                  <i class="el-icon-document"></i>
                  {{ $translateTitle('产品图档') }}
                </bt-dropdown-item>
    ...
    <PopProduct :visible.sync="visibleProduct" :row="currentRow" />
  </div>
</template>
<script>

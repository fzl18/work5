    ...
    methods: {
          ...
          .then(async (res) => {
            ...
            if (res.code == 0) {
              ...
              await this.$api
                ...
                .getDictionary({
                  params: { catalogCode: process.env.VUE_APP_CATALOGCODE },
                })
                .then((res) => {
                  // console.log(res)
              ...
              this.$store.commit('coreRoutes/setRoutes', arr)
            } else {
              const arr = [{}]
              this.$store.commit('coreRoutes/setRoutes', arr)
            }
      ...
      hasChildren(val, prefix) {
        val.forEach((item) => {
          ...
          item.fullPath = item.optRoute ? prefix + item.optRoute : ''
          if (item.icon) {
            item.meta.imgUrl = item.icon
          }
          if (item.children) {
            this.hasChildren(item.children, item.prefix || prefix)
          }
        })
        return val
      },

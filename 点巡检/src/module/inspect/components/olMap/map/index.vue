<template>
  ...
  >
    <a :id="downLoadId" :download="downloadName" />
    <slot v-if="load" />
  </div>
</template>
        ...
        handler(value) {
          ...
          if (rotate) {
            OlMap.setControl(
              'attribution',
              value,
              this.controls.attributionOptions
            )
          }
        },
        immediate: false,
        ...
        this.init().then((res) => {
          if (res === 'success') {
            ...
            this.map.on('click', (r) => {
              ...
              this.map.forEachSmFeatureAtPixel(
                r.pixel,
                (i, e) => {
            ...
            this.map.on('dblclick', (evt) => {
              ...
              if (hit) {
                this.map.forEachSmFeatureAtPixel(
                  evt.pixel,
                  (i, e) => {
                    this.$emit('dblclickfeature', i, e, evt, this.map)
                  },
                  {},
                  evt
                )
                // this.$emit('dblclickfeature', evt, this.map)
              } else {
            ...
            this.map.on('contextmenu', (evt) => {
              const pixel = this.map.getEventPixel(evt.originalEvent)
              const hit = this.map.hasFeatureAtPixel(pixel)
              this.$emit('contextmenu', evt, this.map, hit)
            })
            this.$emit('load')
            this.load = true
          }

  ...
  methods: {
    ...
    initTD() {
      const layerVec = this.initXYZbyURL(
        this.tdVec ||
          `https://t4.tianditu.gov.cn/DataServer?T=vec_w&x={x}&y={y}&l={z}&tk=${config.TD_MAP_KEY}`
      )
      const layerCva = this.initXYZbyURL(
        this.tdCva ||
          `https://t3.tianditu.gov.cn/DataServer?T=cva_w&x={x}&y={y}&l={z}&tk=${config.TD_MAP_KEY}`
      )
    ...
    initTDIMG() {
      const layerImg = this.initXYZbyURL(
        this.tdImg ||
          `https://t4.tianditu.gov.cn/DataServer?T=img_w&x={x}&y={y}&l={z}&tk=${config.TD_MAP_KEY}`
      )
      const layerCia = this.initXYZbyURL(
        this.tdCia ||
          `https://t3.tianditu.gov.cn/DataServer?T=cia_w&x={x}&y={y}&l={z}&tk=${config.TD_MAP_KEY}`
      )

  ...
  async mounted() {
    ...
    this.$api.getDailyCheckDictionary(data3).then((res) => {
      if (res.code === 0) {
        res.data.forEach((item) => {
          item.value = item.dataCode
          // {'dataCode': item.dateCode, 'dataValue': item.dataValue}
          item.label = item.dataValue
    ...
    const val = this.$store.state.inspectFaultClassPageData.updataData
    this.options = this.$store.state.inspectFaultClassPageData.treeData

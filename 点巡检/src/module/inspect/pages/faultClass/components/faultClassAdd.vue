<template>
  <div class="app-container">
    <bt-page title="故障代码新增" subtitle="故障代码基本信息">
...
export default {
  ...
  mounted() {
    // this.options =
    //   this.$store.state.inspectFaultClassPageData.treeData
    ...
    if (
      ...
      this.$store.state.inspectFaultClassPageData.clickTreeData.id
    ) {
      this.form.parentClassId =
        this.$store.state.inspectFaultClassPageData.clickTreeData.id
    }
  },
  methods: {
    ...
    onSubmit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          Api.saveFaultInfo({ data: this.form }).then((res) => {
            if (res.success === 1) {
              this.$msg.success('新增成功')
              // this.setIsComponent('MaterialType')
              this.cancel()
              this.$emit('getTree')
              // this.setIsComponent('addFirstClass')
            } else {
              this.$msg.error(res.errorMsg ? res.errorMsg : '新增失败')
            }
          })
        }
      })
    ...
    uploadfile(o) {
      ...
      Api.uploadFile(params, 'public').then((res) => {
        ...
        if (res.success === 1 && res.data) {
          ...
          this.form.resolventFileAll = res.data.pathAll

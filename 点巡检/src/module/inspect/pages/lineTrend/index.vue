<template>
  <el-main class="si">
    <bt-page :title="$route.meta.title" icon-bg-color="#FF866A">
      <!--      <zPageTitle title="输送线统计" :show-filter="false" />-->
      <div class="s-box">
        <el-tabs @tab-click="handleClick">
          <el-tab-pane
            :label="translateTitle('巡检计划输送线统计')"
          />
          <el-tab-pane
            :label="translateTitle('点检计划输送线统计')"
          />
        </el-tabs>
...
import { getDashLineTrendEchartsData, getDashLineTrendTableData, } from '../../api/lineTrend'
...
export default {
  ...
  data () {
    return {
      ...
      taskType: 1,
      cols: [
        // { prop: 'productionLineName', label: '输送线名称' },
        // { prop: 'factoryDistrictName', label: '所属厂区' },
        { prop: 'departName', label: '所属区域' },
        { prop: 'conveyorName', label: '输送机名称' },
        { prop: 'shouldAction', label: '应巡次数' },
        { prop: 'actulAction', label: '实巡次数' },
        { prop: 'nonChecked', label: '未巡次数' },
        { prop: 'skipped', label: '跳过次数' },
        { prop: 'omitAction', label: '漏检次数' },
        { prop: 'abnormalNum', label: '异常次数' },
        { prop: 'abnormalRate', label: '异常率' },
      ],
      tableData: [],
      total: 0,
      eachPageSize: 10,
  ...
  methods: {
    ...
    myEcharts () {
      ...
      this.echartData.dates &&
      this.echartData.dates.forEach((data, index) => {
        let temp = [data]
        this.echartData.allData.forEach((item) => {
          temp.push(item.trends[index])
        })
        sList.push(temp)
      })
      /* this.echartData.dates && this.echartData.dates.forEach(data => {
              sList.push([data.name,...data.trends])
          }); */
      // config
      let option = {
        legend: {},
        ...
        dataset: {
          dimensions: ['product', ...this.echartData.projectNames],
          source: sList,
        },
        xAxis: { type: 'category' },
        yAxis: {},
        series: [
          { type: 'bar', itemStyle: { color: '#08a0fe' } },
          { type: 'bar', itemStyle: { color: '#309a00' } },
          { type: 'bar', itemStyle: { color: '#ff0000' } },
          { type: 'bar', itemStyle: { color: '#9a00ff' } },
        ],
      }
      myChart.setOption(option, true)
      console.log('echarts done')
    },
    getTargetDay (day) {
    ...
    async getEchartsDataFn (beginTime, endTime, taskType) {
      ...
      await getDashLineTrendEchartsData({
        beginTime,
        endTime,
        taskType,
      }).then((res) => {
    ...
    async getTableDataFn (beginTime, endTime, taskType, pageNum, pageSize) {
      ...
      await getDashLineTrendTableData({
        beginTime,
        endTime,
        taskType,
        pageSize,

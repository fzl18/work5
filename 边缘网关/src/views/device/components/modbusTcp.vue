...
<script>
  ...
  import { isFloatNumber, isLANIP } from '@/utils/formValidatorHelper'
  export default {
    name: 'ModbusTcp',
    inject: {
      orginData: {
    ...
    async created() {
      const connectTypes = await getTypes(TypeName.DISPATCHTYPE)
      const connectMethods = await getTypes(TypeName.DISPATCHMETHOD)
      const bitSorts = await getTypes(TypeName.BYTECOUNT)
      const chatSorts = await getTypes(TypeName.BYTESORT)
      this.$set(this.formData.connectType, 'options', connectTypes)
      this.$set(this.formData.connectMethod, 'options', connectMethods)

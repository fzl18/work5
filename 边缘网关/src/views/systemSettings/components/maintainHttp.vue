...
<script>
  import {
    getPointListByHttpId,
    saveHttpServerDevice,
    savePoint,
    deletePoint,
    batchDeletePoints,
  } from '@/api/http'
  ...
  export default {
    ...
    methods: {
      ...
      async getData() {
        ...
        const data = { ...this.sourceData.orginData }
        this.formData = dataToFormData(data, this.formData)
      },
      async getPointList() {
        if (!this.sourceData.id) return
        const { data } = await getPointListByHttpId(this.sourceData.id)
        this.list = data
      },
      doPointEdit(row) {
      ...
      save() {
        this.$refs['autoForm'].submit().then(async () => {
          const postData = {
            httpServerId: this.sourceData.id,
            ...formDataToPostData(this.formData),
          }
          await saveHttpServerDevice(postData)
      ...
      async savePoint() {
        const postData = {
          topicHttpId: this.sourceData.id,

        ...
        options: {
          ...
          checkChunkUploadedByResponse: function (chunk, message) {
            ...
            try {
              ...
              if (objMessage.data && objMessage.data.uploaded) {
                const result = objMessage.data
                skip = (result.uploaded || []).indexOf(chunk.offset + 1) >= 0
              }
            } catch (e) {}
            return skip
          },
    ...
    methods: {
      ...
      onFileAdded(file) {
        if (file.size / (1024 * 1024) > this.maxSize) {
          this.$baseMessage(
            `${file.name} 文件：${(file.size / (1024 * 1024)).toFixed(
              2
            )}M, 已超过最大限制:${this.maxSize}M！`,
            'error'
          )
          file.ignored = true
        } else {

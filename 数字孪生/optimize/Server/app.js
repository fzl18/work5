...
if (argv.s) {
  config.HttpPort += argv.s;
  config.StreamerPort += argv.s;
}
...
if (config.EnableWebserver) {
  const targetURL = "https://home.bit-plat.com";
  //Setup folders
  ...
  app.use("/frame", proxy({ target: targetURL, changeOrigin: true }));
  app.use("/base", proxy({ target: targetURL, changeOrigin: true }));
  app.use("/framework", proxy({ target: targetURL, changeOrigin: true }));
  app.use("/project", proxy({ target: targetURL, changeOrigin: true }));
  app.use(
    "/cockpit-api/chart",
    proxy({
      target: targetURL,
...
http.listen(httpPort, function () {
  console.logColor(logging.Green, "Http服务侦听端口: " + httpPort);
});

if (config.UseHTTPS) {
  https.listen(httpsPort, function () {
    console.logColor(logging.Green, "Https服务侦听端口*: " + httpsPort);
  });
}

console.logColor(logging.Cyan, `服务运行中.....`);
...
let streamerServerOptions = !config.UseHTTPS
  ...
  : {
      ...
      cert: fs.readFileSync(
        path.join(__dirname, "./certificates/client-cert.pem")
      ), //note this cert must be full chain (i.e. contain domain cert and CA cert)
    };

var streamerHttpServer = require(config.UseHTTPS ? "https" : "http").Server(
  streamerServerOptions
);
streamerHttpServer.listen(streamerPort);
let streamerServer = new WebSocket.Server({
  server: streamerHttpServer,
  backlog: 1,
});

console.logColor(logging.Green, `WebSocket侦听推流端口 on :${streamerPort}`);
let streamer; // WebSocket connected to Streamer

streamerServer.on("connection", function (ws, req) {
  console.logColor(
    logging.Green,
    `推流已连接: ${req.connection.remoteAddress}`
  );
...
console.logColor(logging.Green, `WebSocket侦听用户连接端口:${httpPort}`);
...
playerServer.on("connection", function (ws, req) {
  ...
  if (!streamer || streamer.readyState != 1 /* OPEN */) {
    ws.close(1013 /* Try again later */, "推流未连接");
    return;
  }

  let playerId = (++nextPlayerId).toString();
  console.log(`用户 ${playerId} (${req.connection.remoteAddress}) 已连接`);
  ...
  ws.on("message", function (msg) {
    ...
    if (msg.type == "offer") {
      ...
      console.log(`<- player ${playerId}: ${JSON.stringify(msg.loginInfo)}`);
      if (config.UseMatchmaker) {
        matchmaker.write(JSON.stringify(msg));
      }
  ...
  ws.on("error", function (error) {
    ...
    console.logColor(logging.Red, `尝试重新连接...`);
    reconnect();
  });

  sendPlayerConnectedToFrontend();
  sendPlayerConnectedToMatchmaker(); //todo
...
if (config.UseMatchmaker) {
  ...
  matchmaker.on("connect", function () {
    console.log(
      `服务连接至 Matchmaker ${matchmakerAddress}:${matchmakerPort};`
    );
    ...
    message = {
      ...
      url: argv.url || "", //FRONTEND_WEBSERVER,
    };
  ...
  function reconnect() {
    console.log(`尝试重新连接至 Matchmaker 在 ${matchmakerRetryInterval} 秒`);
    setTimeout(function () {
      connect();

...
let isControl = getUrlParams(window.location.href).ue || true;
...
function sendInputData(data) {
  if (webRtcPlayerObj) {
    resetAfkWarningTimer();
    isControl && webRtcPlayerObj.send(data);
  }
}
...
function setupWebRtcPlayer(htmlElement, config) {
  ...
  webRtcPlayerObj.onWebRtcOffer = function (offer) {
    if (ws && ws.readyState === WS_OPEN_STATE) {
      ...
      ws.send(offerStr);
      ws.send(JSON.stringify({ type: "player", loginInfo }));
    }
  };
...
function connect() {
  ...
  // let wsUrl = process.env.VUE_APP_SOCKET || "ws://localhost";
...
function load(msg) {
  loginInfo = msg.usrInfo.userName;
  setupHtmlEvents();

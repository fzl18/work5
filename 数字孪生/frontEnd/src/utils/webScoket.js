...
function Socket(obj) {
  ...
  this.heartTime = obj.heartTime || 60 * 1000
...
Socket.prototype.addEvent = function (eventName, callback, self, once) {
  if (this.wsCallbackEvent[eventName]) {
    var callbackStatus = this.wsCallbackEvent[eventName].every(function (
      eventData
    ) {
...
Socket.prototype.eventExecute = function (parseData) {
  if (
    parseData &&
    parseData.socketType &&
    this.wsCallbackEvent[parseData.socketType]
  ) {
    var once = []
    this.wsCallbackEvent[parseData.socketType].forEach(function (
      callbackObj,
      index
    ) {
      if (callbackObj.once) {
    ...
    if (once.length) {
      ...
      once.forEach(function (spliceIndex) {
        this.wsCallbackEvent[parseData.socketType].splice(spliceIndex, 1)
      }, this)
    }
  } else {
    if (parseData && parseData.socketType && this.debug) {
...
Socket.prototype.send = function (eventName, param) {
  ...
  if (this.ws && this.ws.readyState == 1) {
    var data = eventName // { method: eventName || '' }

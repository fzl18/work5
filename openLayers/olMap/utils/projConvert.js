...
const sphericalMercator = {
  ...
  forward: forEachPoint(function (input, output, offset) {
    const lat = Math.max(
      Math.min(sphericalMercator.MAX_LATITUDE, input[offset + 1]),
      -sphericalMercator.MAX_LATITUDE
    )
    const sin = Math.sin(lat * sphericalMercator.RAD_PER_DEG)

    output[offset] =
      sphericalMercator.RADIUS * input[offset] * sphericalMercator.RAD_PER_DEG
    output[offset + 1] =
      (sphericalMercator.RADIUS * Math.log((1 + sin) / (1 - sin))) / 2
  }),
  inverse: forEachPoint(function (input, output, offset) {
    output[offset] =
      input[offset] / sphericalMercator.RADIUS / sphericalMercator.RAD_PER_DEG
    output[offset + 1] =
      (2 * Math.atan(Math.exp(input[offset + 1] / sphericalMercator.RADIUS)) -
        Math.PI / 2) /
      sphericalMercator.RAD_PER_DEG
  }),
}
...
const gcj02 = {
  ...
  delta: function (wgLon, wgLat) {
    ...
    dLat =
      (dLat * 180.0) /
      (((this.AXIS * (1 - this.OFFSET)) / (magic * sqrtMagic)) * this.PI)
    dLon =
      (dLon * 180.0) / ((this.AXIS / sqrtMagic) * Math.cos(radLat) * this.PI)
    return [dLon, dLat]
  },

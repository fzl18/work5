...
import { loadModel } from "@/utils/action.js";
export default {
  name: "TTurebine",
  inject: ["global"],
  data() {
  ...
  methods: {
    ...
    changeAnimation(turbine, animationName) {
      ...
      const mixer = new THREE.AnimationMixer(this.matrixTurbine.scene);
    ...
    outline(selectedObjects, color = "#15c5e8") {
    ...
    alarm() {
      ...
      setInterval(() => {
        ...
        const equipment = this.equipmentMaterialMap.get(nameList[random]);
        if (equipment) {
